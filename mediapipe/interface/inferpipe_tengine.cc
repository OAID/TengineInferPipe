...
TengineOperation::~TengineOperation() = default;


InferTenginePipe::InferTenginePipe(){
    mOperation = std::unique_ptr<TengineOperation>(new TengineOperation());
}

InferTenginePipe::~InferTenginePipe() = default;

absl::Status InferTenginePipe::init_pipe(std::string calculator_graph_config_file, std::string input_stream, std::string output_stream){
...
absl::Status InferTenginePipe::infer(const cv::Mat& frame, std::vector<abox>& result){
    cv::Mat camera_frame;
    ...
    // LOG(INFO) << "output_result:"<<output_result.size();
...
absl::Status InferTenginePipe::infer(const cv::Mat& frame, std::vector<acategory>& result){
    cv::Mat camera_frame;
    cv::cvtColor(frame, camera_frame, cv::COLOR_BGR2RGB);
...
absl::Status InferTenginePipe::release_pipe(){
