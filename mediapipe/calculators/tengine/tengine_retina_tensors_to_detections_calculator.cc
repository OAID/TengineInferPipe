...
#include "mediapipe/calculators/tengine/tengine_retina_tensors_to_detections_calculator.pb.h"
...
class TengineTensorsToDetectionsCalculator : public CalculatorBase {
 public:
 ...
 private:
  ...
  ::mediapipe::TengineTensorsToDetectionsCalculatorOptions options_;
...
REGISTER_CALCULATOR(TengineTensorsToDetectionsCalculator);

absl::Status TengineTensorsToDetectionsCalculator::GetContract(
  ...
  auto options = cc->Options<::mediapipe::TengineTensorsToDetectionsCalculatorOptions>();
  ...
  LOG(INFO)<<"TengineTensorsToDetectionsCalculator::GetContract";
  return absl::OkStatus();
}

absl::Status TengineTensorsToDetectionsCalculator::Open(CalculatorContext* cc) {
...
absl::Status TengineTensorsToDetectionsCalculator::Process(
...
std::pair<int,float> TengineTensorsToDetectionsCalculator::argmax(const float * begin, int step){
    float max_val = *begin;
    int max_index = 0;
...
std::pair<int,uint8_t> TengineTensorsToDetectionsCalculator::argmax(const uint8_t * begin, int step){
...
absl::Status TengineTensorsToDetectionsCalculator::ProcessCPU(
  ...
  if (input_tensors.size() == 2 ||
    ...
    const std::vector<float>* raw_score_tensor = &input_tensors[0];
    const std::vector<float>* raw_box_tensor = &input_tensors[1];
    ...
    for (int i = 0; i < num_boxes_; ++i) {
      ...
      if (options_.has_min_score_thresh() &&
        ...
        Detection detection = ConvertToDetection(y1, x1, y2, x2,
          score_val, max_result.first+1, options_.flip_vertically());
...
absl::Status TengineTensorsToDetectionsCalculator::ProcessUintCPU(
    CalculatorContext* cc, std::vector<Detection>* output_detections) {
  const auto& input_tensors =
  ...
  if (input_tensors.size() == 2 ||
    ...
    // Postprocessing on CPU for model without postprocessing op. E.g. output
    // raw score tensor and box tensor. Anchor decoding will be handled below.
    const std::vector<uint8_t>* raw_score_tensor = &input_tensors[0];
    const std::vector<uint8_t>* raw_box_tensor = &input_tensors[1];
    
    const uint8_t* raw_boxes = raw_box_tensor->data();
    const uint8_t* raw_scores = raw_score_tensor->data();

    float score_scale = quant_params[0][0];
    float score_zp = quant_params[0][1];
    float box_scale = quant_params[1][0];
    float box_zp = quant_params[1][1];
    ...
    for (int i = 0; i < num_boxes_; ++i) {
      std::pair<int, uint8_t> max_result = argmax(raw_scores+1, num_classes_-1);
      float score_val = (max_result.second-score_zp)*score_scale;
      if (options_.sigmoid_score()) 
        score_val = 1/(1+exp(0-score_val));
      if (options_.has_min_score_thresh() &&
        score_val > options_.min_score_thresh()) {
        ...
        float tmp_cx = anchor_cx+(*raw_boxes-box_zp)*box_scale *0.1*anchor_w;
        float tmp_cy = anchor_cy+(*(raw_boxes+1)-box_zp)*box_scale *0.1*anchor_h;
        float tmp_w = anchor_w*exp( (*(raw_boxes+2)-box_zp)*box_scale*0.2);
        float tmp_h = anchor_h*exp( (*(raw_boxes+3)-box_zp)*box_scale*0.2);
...
absl::Status TengineTensorsToDetectionsCalculator::Close(CalculatorContext* cc) {

  return absl::OkStatus();
}

absl::Status TengineTensorsToDetectionsCalculator::LoadOptions(
    CalculatorContext* cc) {
  // Get calculator options specified in the graph.
  options_ =
      cc->Options<::mediapipe::TengineTensorsToDetectionsCalculatorOptions>();
...
Detection TengineTensorsToDetectionsCalculator::ConvertToDetection(
